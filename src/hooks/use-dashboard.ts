...
export function useDashboard() {
  ...
  useEffect(() => {
    ...
    // Função para buscar a dica do dia (com cache)
      ...
      try {
        ...
        if (cachedTip && cacheDate === today) {
          ...
          if (recentHistory.length > 0) {
           ...
           setDailyTip(cachedTip); // Usa a dica em cache se estiver offline
        } else {
           setDailyTip("Conecte-se à internet para receber sua primeira dica do dia personalizada!");
        }
      } catch (error) {
        console.error("Failed to fetch daily tip:", error);
        // Fallback para uma dica genérica em caso de erro na API
