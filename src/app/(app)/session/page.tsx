...
function SessionContent() {
  ...
  useEffect(() => {
    ...
    if (foundRoutine) {
        ...
        foundRoutine.exercicios.forEach(ex => {
            initialSets[ex.exercicioId] = Array.from({ length: ex.seriesAlvo }, (_, i) => ({
                id: i,
  ...
  const handleFinishSession = () => {
    ...
    const duracao = Math.round((endTime.getTime() - startTime.current.getTime()) / (1000 * 60));

    const exercicios: ExercicioRegistrado[] = Object.entries(sessionSets)
        .map(([exercicioId, series]) => ({
            exercicioId,
            series: series.filter(s => s.concluido).map(({ id, ...rest }) => rest)
        }))
    ...
    <>
      <PageHeader title={routine.nome} description="Registre sua sessão de treino.">
        <div className="flex gap-2 w-full md:w-auto">
            <Button variant="outline" className="w-1/2 md:w-auto" onClick={handleCancelSession}>
                ...
                <Card key={exercise.exercicioId}>
                    ...
                    <CardContent className="space-y-4">
                        ...
                        {(sessionSets[exercise.exercicioId] || []).map((set, i) => (
                            <div key={set.id} className="grid grid-cols-[auto_1fr_1fr_auto] items-center gap-x-2 sm:gap-x-4">
                        ...
                        <Button variant="secondary" className="w-full" onClick={() => handleAddSet(exercise.exercicioId)}>
                            Adicionar Série
                        </Button>
                    </CardContent>
                </Card>
