...
import { levelNames, levelThresholds } from '@/lib/gamification';
    ...
    return (
        <>
            ...
            <Card className="mb-8">
                <CardHeader>
                    <CardTitle className="flex items-center gap-3">
                        ...
                        Nível Atual: {levelNames[gamification.level]}
                    </CardTitle>
                    <CardDescription>
                    ...
                    <Progress value={progressPercentage} className="h-4" />
                     <div className="flex justify-between text-sm text-muted-foreground mt-2">
                        ...
                        <span className="font-semibold">
                            {xpToNextLevel > 0 ? `${xpToNextLevel.toLocaleString()} XP para o próximo nível` : 'Nível Máximo!'}
                        </span>
                    </div>
                </CardContent>
            </Card>
            ...
            <div className="space-y-4">
                {sortedLevels.map(([level, xpRequired]) => {
                            ...
                            <CardContent className="p-4 flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                     <div className={cn("flex items-center justify-center size-10 rounded-full", isUnlocked ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground")}>
                                        {isUnlocked ? <Check className="size-6" /> : <Lock className="size-5" />}
                                    </div>
                                    <div>
                                        <h3 className={cn("font-bold text-lg", isUnlocked ? "text-primary" : "text-foreground")}>
                                            Nível {level}: {levelNames[levelNumber]}
                                        </h3>
                                        <p className="text-sm text-muted-foreground">
                                            Requer {xpRequired.toLocaleString()} XP
                                        </p>
                                    </div>
                                </div>
                            </CardContent>
