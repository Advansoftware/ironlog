...
const formSchema = z.object({
  ...
  nome: z
    .string()
    .min(3, "O nome da rotina precisa ter pelo menos 3 caracteres."),
  exercicios: z
    .array(exercicioSchema)
    .min(1, "Adicione pelo menos um exercício à rotina."),
});
...
function ManualCreateContent() {
  ...
  const handleExerciseSelect = (index: number, exercicioId: string) => {
    ...
    if (exercicioSelecionado) {
      ...
      form.setValue(
        `exercicios.${index}.nomeExercicio`,
  ...
  const onSubmit = async (values: FormValues) => {
    const rotina: RotinaDeTreino = {
      ...values,
      exercicios: values.exercicios.map((ex) => ({
        ...ex,
        pesoAlvo: ex.pesoAlvo ?? undefined,
      })),
    };
    ...
    try {
      if (isEditMode) {
        ...
        await salvarRotina(newRoutine);
        toast({
          title: "Rotina Salva!",
          description:
            "Sua nova rotina manual foi criada e sincronizada com WGER.",
        });
      }
  ...
  return (
    <>
      <PageHeader
        title={isEditMode ? "Editar Rotina" : "Criar Rotina Manualmente"}
        description="Monte seu treino exatamente como você quer."
      >
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          ...
          <Alert>
            ...
            <AlertDescription>
              <ul className="list-disc pl-5 space-y-1 mt-2 text-xs">
                <li>
                  <strong>Estrutura:</strong> Tente dividir seus treinos por
                  grupos musculares (ex: um dia para peito/tríceps, outro para
                  costas/bíceps, outro para pernas).
                </li>
                <li>
                  <strong>Exercícios:</strong> Comece com 4-6 exercícios por
                  treino. Priorize exercícios compostos (como supino,
                  agachamento, remada) que trabalham vários músculos.
                </li>
                <li>
                  <strong>Séries e Repetições:</strong> 3 séries de 8 a 12
                  repetições é um ótimo ponto de partida para hipertrofia (ganho
                  de massa).
                </li>
                <li>
                  <strong>Consistência:</strong> O mais importante é ser
                  consistente. É melhor treinar 2-3 vezes por semana sem falhar
                  do que tentar ir 5 vezes e desistir.
                </li>
              </ul>
            </AlertDescription>
          </Alert>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Adicione e configure os exercícios da sua rotina.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              {form.formState.errors.exercicios &&
                !form.formState.errors.exercicios.root && (
          ...
          <div className="flex justify-end">
            <Button type="submit">
              {isEditMode ? "Salvar Alterações" : "Salvar Rotina"}
            </Button>
          </div>
        </form>
      </Form>
