...
export default function EvolutionPage() {
  ...
  useEffect(() => {
    async function startConversation() {
      ...
      try {
        ...
        setMessages([{ role: 'ia', content: 'Olá! Parabéns por evoluir. Como podemos ajustar seu treino para continuar progredindo?' }]);
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const handleSendMessage = async (messageContent: string) => {
    ...
    try {
        ...
        setMessages(prev => [...prev, { role: 'ia', content: 'Desculpe, não consegui processar sua solicitação. Poderia tentar novamente?' }]);
    } finally {
        setIsLoading(false);
    }
  }
  ...
  const applyPlan = (plan: PlanoDeAcao) => {
    try {
      ...
      plan.rotinasParaModificar?.forEach(rotina => atualizarRotina(rotina as RotinaDeTreino));
      ...
      const confirmationMessage = "Plano aplicado com sucesso! Podemos fazer mais algum ajuste ou está tudo certo?";
      setMessages(prev => [...prev, { role: 'ia', content: confirmationMessage }]);
      setIsAwaitingPlanConfirmation(false);
                ...
                {msg.plan && (
                    <Card className="mt-4 bg-background/50">
                        ...
                        <CardContent className="text-sm text-muted-foreground space-y-2">
                            {msg.plan.rotinasParaCriar && msg.plan.rotinasParaCriar.length > 0 && <div><strong className="text-foreground">Criar:</strong> {msg.plan.rotinasParaCriar.map(r => r.nome).join(', ')}</div>}
                            {msg.plan.rotinasParaModificar && msg.plan.rotinasParaModificar.length > 0 && <div><strong className="text-foreground">Modificar:</strong> {msg.plan.rotinasParaModificar.map(r => r.nome).join(', ')}</div>}
                            {msg.plan.rotinasParaRemover && msg.plan.rotinasParaRemover.length > 0 && <div><strong className="text-foreground">Remover:</strong> {msg.plan.rotinasParaRemover.length} rotina(s)</div>}
                        </CardContent>
                        <CardFooter>
                             <Button size="sm" onClick={() => applyPlan(msg.plan!)} disabled={!isAwaitingPlanConfirmation}>
